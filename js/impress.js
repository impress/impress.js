...
(function( document, window ){
    ...
    if(body.circle != undefined){
        ...
        var r     = 0;
        if(isNaN(parseInt(body.circle))){
            r = steps.length * 200 - 200;
        }else{
            r = parseInt(body.circle);
        }
        for (var i = 0; i < steps.length; i++) {
            ...
            if(steps[i].getAttribute('data-x') == undefined){
                steps[i].setAttribute("data-x", Math.round(x).toString());
            }
            if (steps[i].getAttribute('data-y') == undefined){
                steps[i].setAttribute("data-y", Math.round(y).toString());
            }
            if(steps[i].getAttribute('data-rotate-z') == undefined){
